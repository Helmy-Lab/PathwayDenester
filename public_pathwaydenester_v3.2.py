...
gmt_data = read_file(gmt_file, isdict = True, skiprows = 0, colorder = [], minlength = 2, dict_key = 0, coment_line_char = '#', sep = '\t')
...
for index, path in approved_pathways.iterrows():
    ...
    if 'intersection' in path_slice.columns:
...
if tranlator_gene_names != '':
    ...
    gene_translator = read_file(splitted[0], isdict = True, skiprows = 1, colorder = col_to_position, minlength = 2, dict_key = col_from_position, coment_line_char = '', sep = split_char)
    for line in range(len(pathways_dictionaries)):
        new_top_10 = []
...
print('calculations over : ' + str(time.time() - start))
